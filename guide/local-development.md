...
Install these before the first `make dev` / `.\scripts\dev-local.ps1`:
...
| PostgreSQL only (Docker) | `make dev-db` | `docker compose up db -d` |
...
| Stop Docker services | `make down` | `docker compose down` |
...
| Migrations hang or fail | PostgreSQL container is not healthy — run `make dev-db` and wait for `pg_isready` |
