...
7. Rebuild so containers pick up code:

```bash
make up
```

**Windows:** see [guide/docker.md](guide/docker.md) for `docker compose` equivalents.
...
| New migration | `make migrate-create MSG="..."` | see [guide/docker.md](guide/docker.md) |
| Apply migrations | `make migrate` | `docker compose exec backend alembic upgrade head` |
| Run tests | `make test` | `docker compose exec backend pytest -v` (+ frontend) |
| Reset DB + stack | `make reset` | `docker compose down -v; docker compose up --build -d` |
| Logs | `make logs` | `docker compose logs -f` |

After branch switch: `make down` → `git checkout <branch>` → `make reset`.
...
- [ ] Containers rebuilt (`make up`) after code changes
...
- Document or assume local conda/Node app runs — workshop is Docker-only
